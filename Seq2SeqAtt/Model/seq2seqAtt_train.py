from .seq2seqAtt_model import AttentionModel
...
class Train:
        ...
        for batch_x, batch_y, y_mask in validation_sampler:
            result, acc, attention_weights = session.run(
                [self.tf_model.pred_argmax,
                 self.tf_model.batch_accuracy,
                 self.tf_model.attention_weights],
                feed_dict={
                    self.tf_model.x: batch_x,
                    self.tf_model.y: batch_y,
                    self.tf_model.y_masks: y_mask,
    ...
    def render_weights(self, attention, input_sent, output_sent, step, sample):
        # input attention shape (output_dim, batch, input_dim, 1)
        #attention = np.squeeze(np.transpose(attention,[1,0,2,3]), -1)
        ...
        plt.savefig(os.path.join(
            self.logs_path, "render/attention_sample."+str(step)+"_"+str(sample)+".png"))
        with open(os.path.join(self.logs_path, "render/attention_dump"+str(step)+"_"+str(sample)), "w") as f:
            np.savetxt(f, attention)
            f.write("\n\n")
            f.write(" ".join(input_sent))
            f.write("\n\n")
            f.write(" ".join(output_sent))
        self.render_cnt += 1
